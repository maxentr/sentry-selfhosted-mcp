...
export function register(server: McpServer, api: ApiClient, orgSlug: string) {
  server.tool(
    ...
    'List issues for a specific project, optionally filtering by query or status. Supports pagination for large result sets.',
    {
      ...
      query: z.string().optional().describe('Optional Sentry search query (e.g., "is:unresolved environment:production").'),
      status: z.enum(['resolved', 'unresolved', 'ignored']).optional().describe('Optional issue status filter.'),
      limit: z.number().min(1).max(100).optional().describe('Maximum number of issues to return (1-100, default: 25).'),
      cursor: z.string().optional().describe('Pagination cursor from previous response for getting next page.'),
    },
    async (args) => {
