...
export function register(server: McpServer, api: ApiClient, orgSlug: string) {
  server.tool(
    ...
    'Check if debug symbols (dSYM files) are missing for iOS/macOS crashes. Missing dSYMs cause stack traces to show addresses instead of function names. Returns list of missing symbols with UUIDs for upload.',
    {
      ...
      event_id: z.string().optional().describe('Optional: Specific event ID to check. If not provided, checks recent events in project.'),
    },
    async (args) => {
      ...
      if (args.event_id) {
        eventData = await api.get(`projects/${orgSlug}/${args.project_slug}/events/${args.event_id}/`);
      } else {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const issues = await api.get<any[]>(`projects/${orgSlug}/${args.project_slug}/issues/`, { limit: 1 });

        if (!issues || issues.length === 0) {
          return {
            content: [{ type: 'text' as const, text: 'No recent issues found in project. Cannot check dSYM status.' }],
          };
        }
