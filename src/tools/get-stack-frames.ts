...
export function register(server: McpServer, api: ApiClient, orgSlug: string) {
  server.tool(
    ...
    'Extract structured stack trace frames from an event. Optimized for debugging - returns only relevant frame info (function, file, line, in_app status) without noise. Much more efficient than raw_sentry_api for stack trace analysis.',
    {
      ...
      in_app_only: z.boolean().default(false).describe('Filter to only show frames from your application code (excludes system/library frames). Default: false'),
      max_frames: z.number().default(50).describe('Maximum number of frames to return. Default: 50. Start from most recent (bottom of stack).'),
    },
    async (args) => {
      console.error(`Extracting stack frames from event ${args.event_id} in project ${args.project_slug}`);

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const eventData = await api.get<any>(`projects/${orgSlug}/${args.project_slug}/events/${args.event_id}/`);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const frames: any[] = [];
